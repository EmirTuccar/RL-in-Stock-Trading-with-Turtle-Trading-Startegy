...
class TurtleTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Auto-pyramid logic: add 1 unit each +0.5 ATR move from last pyramid_price
        ...
        if action in [Actions.Sell1.value, Actions.Sell2.value]:
            if price < low_10:
                self._sell(1 if action == Actions.Sell1.value else 2, price)
            else:
                print("❌ Turtle SELL rule not triggered")

        if self.position > 0 and self.last_buy_price and price < self.last_buy_price - 2 * atr:
        ...
        if action in [Actions.Buy1.value, Actions.Buy2.value] and self.last_buy_price and abs(price - self.last_buy_price) < 0.5:
            reward -= 5

        self.total_reward += reward

        obs = self._get_obs()
        print(f"Price: {price:.2f}, High_20: {high_20:.2f}, ATR: {atr:.2f}, LastBuy: {self.last_buy_price}")
    ...
    def _update_profit(self, price):
        self.total_profit = self.cash + self.position * price
